...
| 通知の繰り返し表示 | クリック/クローズまで約1分間隔で再表示（既定 ON・最長24時間） |
| 差出人/件名の表示 | 通知に差出人・件名を含めるか（既定 OFF・判定理由のみ表示） |

### 通知のプライバシーと持続表示

- 既定では通知に**差出人・件名・本文を一切表示せず**、LLM の判定理由のみを
  表示します（理由文にも本文の具体的内容を含めないよう LLM に指示しています）。
- OS の通知バナーの表示時間は拡張から制御できないため、「繰り返し表示」は
  未確認の通知を約1分間隔で再表示することで実現しています。
  **通知をクリック**（メールが開く）または **✕ で閉じる**と確認済みとみなして
  停止します。未確認でも 24 時間で自動停止します。
...
- MV3 の非永続バックグラウンドはアイドル時にアンロードされますが、新着イベントや
  アラームで起床します。保留中の通知は storage.local に永続化しているため、
  時間が経った通知をクリックしても該当メールを開けます。
- 通知バナー自体の表示秒数は OS の設定に依存します（GNOME や Windows の
  通知設定で延長可能）。本アドオンの「繰り返し表示」は再表示による擬似的な持続です。
